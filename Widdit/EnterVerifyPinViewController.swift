        ...
        verification.verify(pinText.text!) { (success, err) in
            if success {
                ...
                query.findObjectsInBackgroundWithBlock({ (objects: [PFObject]?, err: NSError?) in
                    if objects?.count > 0 {
                        let alert = UIAlertController(title: "Sorry", message: "Looks like your phone number is already in the database. Please use a different phone number.", preferredStyle: .Alert)
